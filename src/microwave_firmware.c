/*
 * ПОВНА ПРОШИВКА МІКРОХВИЛЬОВКИ (v_final_2.8.0_refactored)
 * Модель: LG MS-2044J (на базі ATmega8, 16/8МГц)
 *
 * --- ОПИС ФУНКЦІОНАЛУ v2.8.0 ---
 * 1.  (v2.8.0) РЕФАКТОРИНГ: Весь код розбито на логічні модулі:
 * - microwave_firmware.c: (Цей файл) Тільки головний цикл (loop),
 * автомат станів (handle_state_machine)
 * та визначення глобальних змінних.
 * - auto_programs.c/h:    Всі PROGMEM таблиці та логіка авто-програм.
 * - cooking_logic.c/h:    Все керування ШІМ, магнетроном, таймером готування.
 * - timers_isr.c/h:       Всі ISR, налаштування таймерів, годинник, біпери.
 * - display_driver.c/h:   (Без змін)
 * - keypad_driver.c/h:    (Без змін)
 * 2.  (v2.8.0) ВИПРАВЛЕНО: Усі функції (v2.7.0) збережено.
 */
...
void loop() {
    ...
    if (g_state == STATE_STAGE2_TRANSITION) {
        ...
        if(g_state==STATE_SLEEPING) { 
            reset_to_idle(); // (v2.8.0) Переконуємося, що все скинуто
            enter_sleep_mode(); 
            sleep_cpu(); 
            s_lps=0; 
    ...
    if(g_state!=STATE_SLEEPING) {
        ...
        // (v2.8.0) Покращена обробка скидання
        if(g_state==STATE_FINISHED || g_state==STATE_POST_COOK) {
             // Якщо стан змінився на IDLE (з run_1sec_tasks) або
             // користувач натиснув СТОП, викликаємо reset_to_idle()
            if (key == KEY_STOP_RESET) {
                 reset_to_idle(); 
                 if (allow_beep) do_short_beep();
